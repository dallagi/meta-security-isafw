...
source_to_bins_file = '/isafw_dep_tmp_stob'
...
class ISA_DEPChecker:
    ...
    def __init__(self, ISA_config):
        ...
        self.stob = None  # map run-time source packages to the binary packages they generate
    ...
    def process_package(self, ISA_pkg):
        ...
        r_deps = dict()
        bins = set()  # binaries generated from this source package (if it is a source package)
        ...
        if bins:
            with open(self.reportdir + source_to_bins_file, 'a+b') as f:
                obj = (ISA_pkg.name, bins)
                pickle.dump(obj, f)

    def load_source_to_bins_mapping(self):
        """ Fill the self.stob dictionary
        The self.stob dictionary maps a source package name
        to the names of the binary packages that it builds
        """
        tmp = open(self.reportdir + source_to_bins_file, 'rb')
        self.stob = {}
    ...
    def sources_to_bins(self, source_pkgs):
        """
        Given a list of source package names,
        return the list of names of the binary packages
        generated by those source packages

        No errors will be raised in case of non-existent
        or misspelled source package name(s)
        """
        if self.stob is None:
            self.load_source_to_bins_mapping()

        bins = set()
        for source_pkg in source_pkgs:
            bins.update(self.stob.get(source_pkg, set()))
    ...
    def cleanup(self):
        try:  # remove temp file
            # todo: remove source_to_bin mapping file?
            os.remove(self.reportdir + temp_file)
        except OSError:
            pass
